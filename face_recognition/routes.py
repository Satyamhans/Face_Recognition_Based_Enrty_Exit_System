...
def registration():
    ...
    if form.validate_on_submit():
        roll_no=form.enrol_number.data
        ...
        if(orm.register_face(roll_no)):            
            student = Student_details(enrol=form.enrol_number.data, name=form.name.data, contact=form.contact.data, parent_contact=form.parent_contact.data, room_num=form.room_number.data)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if username=="admin" and password=="1234":
            # user=Student_details.query.filter_by(name=username).first()
            # if user.enrol=="admin2017":
            #     login_user(user)
